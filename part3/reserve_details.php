...
if (isset($_POST['borrow'])) {
    ...
    $stmt = $conn->prepare("SELECT volume FROM books WHERE book_id=?");
    ...
    if (!$book || $book['volume'] <= 0) {
        $_SESSION['message'] = "Cannot borrow: Book is out of stock.";
    ...
    if (!$exists) {
        $stmt = $conn->prepare("INSERT INTO borrower (borrower_id, borrower_name, course, year, borrower_type) VALUES (?, ?, ?, ?, ?)");
    ...
    // Insert transaction
    $stmt = $conn->prepare("INSERT INTO transactions (book_id, borrower_id, date_borrowed) VALUES (?, ?, NOW())");
    $stmt->bind_param("ii", $book_id, $borrower_id);
    $stmt->execute();
    $stmt->close();

    // Reduce book volume
    $stmt = $conn->prepare("UPDATE books SET volume = volume - 1, status = IF(volume - 1 > 0, 'Available', 'Out of Stock') WHERE book_id=?");
    $stmt->bind_param("i", $book_id);
    $stmt->execute();
    $stmt->close();
...
while ($row = $expired->fetch_assoc()) {
    ...
    $borrower_id   = intval($row['borrower_id']);
    $borrower_name = trim($row['borrower_name']);
    $course        = trim($row['course']);
    $year          = intval($row['year']);
    $type          = trim($row['borrower_type']);
    $book_id       = intval($row['book_id']);

    // Check again if book still has volume
    $stmt = $conn->prepare("SELECT volume FROM books WHERE book_id=?");
    $stmt->bind_param("i", $book_id);
    $stmt->execute();
    $book = $stmt->get_result()->fetch_assoc();
    ...
    if (!$book || $book['volume'] <= 0) continue; // skip if out of stock
...
/* ===============================
   FETCH RESERVATIONS
================================ */
$reservations = $conn->query("SELECT * FROM reservations ORDER BY reserve_date DESC");
?>
