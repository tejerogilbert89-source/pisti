...
<tr>
    ...
    <td><?= !empty($row['date_borrowed']) ? date('Y-m-d H:i:s', strtotime($row['date_borrowed'])) : '-' ?></td>

    <td style="color:<?= $lateFee > 0 ? 'red' : 'inherit' ?>">
        <?= !empty($row['due_date']) ? date('Y-m-d H:i:s', strtotime($row['due_date'])) : '-' ?>
        <?= ($lateFee > 0 && empty($row['date_returned'])) ? '<br><strong>OVERDUE</strong>' : '' ?>
    </td>

    <td>
        <?= !empty($row['date_returned']) 
            ? date('Y-m-d H:i:s', strtotime($row['date_returned'])) 
