...
if (isset($_POST['reserve'])) {
    ...
    if ($borrower_type === "Student") {
        ...
        } elseif (empty($year) || !in_array($year, $valid_years)) {
    ...
    switch ($borrow_period) {
        case '1 day':
            $due_date = date('Y-m-d H:i:s', strtotime($reserve_date.' +1 day'));
            break;
        case '3 days':
            $due_date = date('Y-m-d H:i:s', strtotime($reserve_date.' +3 days'));
            break;
        case '7 days':
            $due_date = date('Y-m-d H:i:s', strtotime($reserve_date.' +7 days'));
            break;
        case '1 month':
            $due_date = date('Y-m-d H:i:s', strtotime($reserve_date.' +30 days'));
            break;
        default:
            $due_date = date('Y-m-d H:i:s', strtotime($reserve_date.' +7 days'));
    }
...
<input type="text" name="last_name" required pattern="[A-Za-z ]+">

<label>Borrower ID</label>
<input type="text" name="borrower_id" id="borrower_id" required pattern="[0-9]+">
...
function toggleCourseYear() {
    ...
    if (type === 'Teacher') {
        ...
        borrowerId.maxLength = 5;
    } else {
        courseYear.style.display = 'block';
        courseSelect.disabled = false;
        yearSelect.disabled = false;
        borrowerId.maxLength = 7;
    }
}
